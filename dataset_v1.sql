...
where  anio_cancel = 2022) as filtro
...
on filtro.account_no = modems.cuenta
...
SELECT cf.idcuentabrm_c,count(cf.idcuentabrm_c), s.d_codigo, s.d_estado, s.d_asenta AS colonia
FROM nuevos_negocios.sepomex s JOIN data_staging.slf_cuentafactura__c cf
     ON s.d_codigo=cf.codigopostalinstalacion__c
WHERE cf.info_day = 20221030
group by cf.idcuentabrm__c,s.d_codigo, s.d_estado,colonia
limit 100
